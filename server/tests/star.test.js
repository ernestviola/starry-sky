...
describe('GET /api/stars success', () => {
  ...
  describe('unsuccessful', () => {
    test('rejects with empty frame', async () => {
      const response = await request(app).get('/api/stars/frame').expect(400);

      expect(response.body.success).toBe(false);
    });

    test('rejects with invalid frame value', async () => {
      const response = await request(app)
        .get('/api/stars/frame?frames=abc')
        .expect(400);
      expect(response.body.success).toBe(false);
    });
  });
